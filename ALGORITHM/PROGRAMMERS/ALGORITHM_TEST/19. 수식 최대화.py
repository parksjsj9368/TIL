def solution(expression):
    ...
    max_val = 0
    for i in oper_list:  # i가 (*, +, -) ...
        oper1 = i[0]
        oper2 = i[1]

        temp_list = []
        for j in expression.split(oper1):  # *로 먼저 쪼개
            temp = [f'({k})' for k in j.split(oper2)]  # -로 쪼개, 젤 큰 연산자 ()로 묶기
            temp_list.append(f'({oper2.join(temp)})')  # 두번째 큰 연산자 ()로 묶기
        expr = oper1.join(temp_list)

        max_val = max(max_val, abs(eval(expr)))
    return max_val
